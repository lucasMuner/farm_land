...
void draw() {
  ...
  if(!gameStarted)
  {
    ...
    // Tela de vitória
    rectMode(CORNER);
    fill(150, 75, 0, 150); // Fundo preto meio transparente
  ...
  for (int y = 0; y < rows; y++) {
    for (int x = 0; x < cols; x++) {
      ...
      if (pixelColor == black) {
        ...
        floors.add(new Floor(x * wallSize * scale, y * wallSize * scale, wallSize * scale, wallSize * scale, floorImage));
        player = new Player(x * wallSize * scale, y * wallSize * scale, playerSize * scale, speed, playerImage, spriteSheet,inventory, playerMoney, menu.difficulty);
